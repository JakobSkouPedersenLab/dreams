  ...
  for (batch in sort(unique(bam_df_batches$batch_idx))) {
    ...
    read_feature_df <-
      ...
      mutate(
        ...
        q1 = substring(.data$qual, .data$pos_idx, .data$pos_idx),
        q2 = utf8ToInt(q1),
        q3 = q2 - 33
      ) %>%
    ...
    selected_features <-
      c(
        ...
        "n_other_errors", "n_insertions_in_read", "n_deletions_in_read", "seq_length", "mapq", "q","q1", "q2", "q3", "qual"
      )
    ...
    feature_data <- rbind(feature_data, feature_df)
  }
